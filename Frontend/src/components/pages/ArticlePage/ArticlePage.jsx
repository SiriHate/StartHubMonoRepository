...
function publicationOwnerUsername(owner) {
    if (owner == null) return '';
    if (typeof owner === 'string') return owner;
    if (typeof owner === 'object' && owner.username != null) return String(owner.username);
    return '';
}
...
const ArticlePage = () => {
    ...
    const handleLike = async () => {
        const authorUsername = publicationOwnerUsername(article?.owner);
        if (
            currentUser?.username &&
            authorUsername !== '' &&
            String(currentUser.username).toLowerCase() === authorUsername.toLowerCase()
        ) {
            return;
        }
        try {
    ...
    const handleApprove = async () => {
        try {
            const response = await apiClient(`${config.MAIN_SERVICE}/articles/${articleId}/moderation-status`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
