...
function ManageProject() {
                ...
                if (response.ok) {
                    ...
                    SEARCH_TYPES.forEach(st => {
                        (data[st.field] || []).forEach(item => {
                            ...
                            } else if (st.key === 'investors') {
                                loaded.push({ id: item.id, type: st.key, about: item.about || '', _saved: true });
                            } else {
    ...
    const openSearchForm = (type) => {
        ...
        if (type === 'investors' && searches.some(s => s.type === 'investors')) return;
        setEditingIndex(null);
        setEditingSearch({
            type,
            about: '',
            ...(type === 'employees' ? { specialization: '' } : {}),
            ...(type === 'founders' || type === 'mentors' ? { domain: '' } : {}),
        });
    };
    ...
    const handleSaveSearch = async () => {
        ...
        const body = editingSearch.type === 'employees'
            ...
            : (editingSearch.type === 'investors'
                ? { about: editingSearch.about }
    ...
    const hasInvestorSearch = searches.some(s => s.type === 'investors');
                ...
                <div className={styles.container}>
                    ...
                    <div className={styles.card}>
                        ...
                        {!editingSearch && (
                            <div className={styles.searchTypeRow}>
                                {SEARCH_TYPES.map(st => (
                                    <button key={st.key}
                                            type="button"
                                            className={styles.addSearchBtn}
                                            onClick={() => openSearchForm(st.key)}
                                            disabled={st.key === 'investors' && hasInvestorSearch}>
                        ...
                        {editingSearch && (() => {
                            ...
                            const isInvestor = editingSearch.type === 'investors';
                            return (
