...
import org.siri_hate.chat_service.dto.ChatFullResponseDTO;
import org.siri_hate.chat_service.dto.ChatMemberRequestDTO;
import org.siri_hate.chat_service.dto.ChatPageResponseDTO;
import org.siri_hate.chat_service.dto.ChatRequestDTO;
import org.siri_hate.chat_service.dto.ChatSummaryResponseDTO;
import org.siri_hate.chat_service.dto.ChatTypeDTO;
...
public class ChatService {
    ...
    public ChatFullResponseDTO createChat(String creatorUsername, ChatRequestDTO request) {
        if (ChatTypeDTO.PRIVATE.equals(request.getType())
                && request.getMembers() != null
                && request.getMembers().size() == 1) {
            String otherUsername = request.getMembers().get(0).getUsername();
            if (otherUsername != null && !otherUsername.equalsIgnoreCase(creatorUsername)) {
                List<Chat> existing = chatRepository.findPrivateChatsBetweenTwoUsers(
                        ChatType.PRIVATE,
                        creatorUsername,
                        otherUsername
                );
